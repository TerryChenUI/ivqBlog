angular.module('common.util')
    .factory('Tool', ['$window', ($window) => {
            class ToolUtil {
                trimSameProperties(initial, modified) {
                    angular.forEach(modified, (val, name) => {
                        if (initial.hasOwnProperty(name) && _.isObject(modified[name])) {
                            this.trimSameProperties(initial[name], modified[name]);
                ...
                transformArticleUrl(data) {
                    let url = "http://" + $window.location.host + "/post/" + data.category.route + "/" + data._id;
